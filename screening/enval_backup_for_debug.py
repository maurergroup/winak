...
class FabioPopEvaluator(PopulationEvaluator):
    ...
    def EvaluatePopulation(self,popreceived):
        ...
        OptimizedPopulation = self.__eliminate_duplicates(OptimizedPopulation,EvaluatedPopulation)
    ...
    def __check_identity(self,a,b):
        ...
        print("________")#
        print("_check_identity called")#
        #first, check stoichiometry
        if a.get_chemical_formula() != b.get_chemical_formula():
            print("different formulas")#
            return False
        
        #second, check energy
        if abs(a.info["fitness"] - b.info["fitness"]) > 0.1:
            print("differenza maggiore del limite:")#
            print(abs(a.info["fitness"] - b.info["fitness"])) #
            return False
        ...
        if 0==1:            # metodofighissimo(a,b) < 0.99:
            return False
        print("strutture trovate identiche")#
        return True
    ...
    def __eliminate_duplicates(self,NewPop,OldPop):
        ...
        print("eliminate_duplicates called")#
        print("NewPop is:")#
        for cosina in NewPop:#
            print(cosina.get_chemical_formula(),cosina.info["fitness"])
        to_remove=[]
        for stru in NewPop:
            print("now to_remove is:")#
            for analysis in to_remove:#
                print(analysis.get_chemical_formula(),analysis.info["fitness"])#
            if stru not in to_remove:
                print("Take in consideration ",stru.get_chemical_formula(),stru.info["fitness"],'it is not in to_remove')#
                all_the_others = NewPop[:]
                all_the_others.remove(stru)
                print("all the others is:")#
                for analysis in all_the_others:#
                    print(analysis.get_chemical_formula(),analysis.info["fitness"])#
                for stru2 in all_the_others:
                    print("confront with:")#
                    print(stru2.get_chemical_formula(),stru2.info["fitness"])#
                    if stru2 not in to_remove:
                        if self.__check_identity(stru,stru2):
                            to_remove.append(stru2.copy())
                            print("now to remove is:")#
                            for thing in to_remove:#
                                print(thing.get_chemical_formula(),thing.info["fitness"])#
                        
        for stru in to_remove:
            print("now removing:")#
            print(stru.get_chemical_formula())#
            NewPop.remove(stru)
            print("now NewPop is:")#
            for roba in NewPop:#
                print(roba.get_chemical_formula(),roba.info["fitness"])#
        
        #Eliminates all structures already in OldPop
        popcopy = NewPop[:]
        print("confront old and new:")#
        for stru in popcopy:
            print("1",stru.get_chemical_formula(),stru.info["fitness"])#
            for stru2 in OldPop:
                print('2',stru2.get_chemical_formula(),stru.info["fitness"])#
                if self.__check_identity(stru,stru2):
                    NewPop.remove(stru)
                    print("now NewPop is:")#
                    for roba in NewPop:#
