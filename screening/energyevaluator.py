...
class FabioPopEvaluator(PopulationEvaluator):
    ...
    def EvaluatePopulation(self,popreceived):
        ...
        OptimizedPopulation = self.eliminate_duplicates(OptimizedPopulation,EvaluatedPopulation)
    ...
    def check_identity(self,a,b):
        ...
        if (a.info["fitness"] - b.info["fitness"]) > 0.1:
            return False
        #third, check structure. It only confronts the interatomic distances between atoms of the same element.
        #I consider it reasonable, because we are confronting two structures with very similar energy.
#####WRITE
        return True
    
    def eliminate_duplicates(self,NewPop,OldPop):
        ...
        for stru in NewPop:
            if stru not in to_remove:
                ...
                for stru2 in all_the_others:
                    if self.check_identity(stru,stru2):
