...
!
! The following is the original copyright notice from the source package
! icfs-1.4.tar.gz downloaded from:
!
...
! *****************************************************************
     ...
     +                 p,alpha,iwa,wa1,wa2,offset,info)
      integer n, nnz, p, offset, info
...
c     Compute the l2 norms of the columns of A.
      ...
      do j = 1, n
         do i = acol_ptr(j)+offset, acol_ptr(j+1)-1+offset
            k = arow_ind(i)+offset
            wa1(j) = wa1(j) + a(i)**2
            wa1(k) = wa1(k) + a(i)**2
         end do
...
c        Copy the sparsity structure of A into L.
         ...
         do i = 1, n+1
            lcol_ptr(i) = acol_ptr(i)+offset
         end do
         do i = 1, nnz
            lrow_ind(i) = arow_ind(i)+offset
         end do
...
c        Scale A and store in the lower triangular matrix L.
         ...
         do j = 1, n
            do i = acol_ptr(j)+offset, acol_ptr(j+1)-1+offset
               l(i) = a(i)*wa2(j)*wa2(arow_ind(i)+offset)
            end do
         end do
