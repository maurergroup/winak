...
class EE:
    ...
    def __init__(self,optlog='opt.log'):
    ...
    def get_energy(self,atoms):
        """subclasses must implement this method. Has to return an ase.atoms object
...
class potEE(EE):
    ...
    def __init__(self,opt,fmax,opt2=None,fmax2=None,optlog='opt.log'):
        EE.__init__(self, optlog)
        self.fmax=fmax
        self.opt=opt
        self.fmax2=fmax2
        self.opt2=opt2        
    ...
    def print_params(self):
        ret='%s: Optimizer=%s, fmax=%f'%(self.__class__.__name__,self.opt.__class__.__name__,self.fmax)
        if self.opt2 is not None and self.fmax2 is not None:
            ret+=', Optimizer2=%s, fmax2=%f '%(self.opt2.__class__.__name__,self.fmax2)
        return ret